-- 109_pc_otif_real_desde_rechazos.sql
-- Corrige el cálculo del OTIF en el calendario de Períodos Críticos (Pampeana).
--
-- Antes: OTIF = 1 - hl_rechazo / hl_vendido(cron). Como el HL del cron es de otro
-- universo (mayor), la tasa de rechazo salía ínfima y trigger_otif nunca gatillaba.
-- Ahora: OTIF = 1 - bultos_rechazados / bultos_entregados de la propia tabla
-- `rechazos` (mismo universo), que es la definición correcta de nivel de servicio.
-- Se recalibra otif_min a 0.97 (rechazo > 3% = día con problema de servicio); con
-- 0.92 (rechazo > 8%) casi nunca gatillaba dado el rechazo bajo de Pampeana (~0.9%).
...
select anio, fecha, dow,
  ...
  case when dow = 0 then 'NORMAL' when trigger_count >= (select min_triggers from cfg) then 'CRITICO' else 'NORMAL' end as estatus,
  case when dow = 0 then 'BAJO' when score >= (select umbral_score_alto from cfg) then 'ALTO'
       when score >= (select umbral_score_medio from cfg) then 'MEDIO' else 'BAJO' end as nivel
from scored order by anio, fecha;

update pc_umbrales set otif_min = 0.970, updated_at = now() where id = 1;
