-- 111_pc_nivel_por_contexto.sql
-- El NIVEL de severidad de un día crítico pasa a reflejar el contexto operativo:
-- un día de pico (volumen/clientes) que además cae en un mes de alto ausentismo
-- o con OTIF bajo (o que es pico DOBLE volumen+clientes) se marca ALTO; el resto
-- de los días críticos quedan MEDIO. Antes el nivel salía del score ponderado, que
-- con el ausentismo bajo casi no movía. Así el ausentismo y el OTIF influyen de
-- verdad en la severidad sin crear días críticos de más.
