-- 113_pc_critico_por_multiples_condicionantes.sql
-- Día CRÍTICO = al menos `min_triggers` (=2) condicionantes el MISMO día
-- (volumen, clientes, OTIF, ausentismo). Antes era pico operativo (vol o cli),
-- pero ahora que OTIF y ausentismo son DIARIOS se puede exigir coincidencia real
-- sin pintar meses enteros. Nivel: 3+ condicionantes = ALTO, 2 = MEDIO.
-- Cumple R3.4.1 (3 variables del manual + mínimo 3 períodos): da ~7 días/año
-- agrupados en 3-4 períodos.
...
select anio, fecha, dow,
  ...
  case when dow = 0 then 'BAJO'
       when trigger_count < (select min_triggers from cfg) then 'BAJO'
       when trigger_count >= 3 then 'ALTO'
