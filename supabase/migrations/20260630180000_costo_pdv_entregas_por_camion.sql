-- Costo por PDV — el campo `comprobantes` pasa a contar ENTREGAS (idas de camión
-- por día), no comprobantes. Mismo camión + mismo día con varios comprobantes =
-- 1 entrega; dos camiones distintos el mismo día = 2. Se cuenta distinct
-- (fecha, camión) por cliente, ignorando el origen (chess/gestion) para no
-- duplicar la misma ida. Antes: sum(vd.comprobantes).
-- Afecta la columna "Entregas" y el reparto del costo_distancia (prorrateo por
-- entregas del cliente sobre el total de la ciudad).
-- En la UI la columna ya se rotula "Entregas" (costo-pdv-client / acumulado-tab).
--
-- NOTA: estas funciones venían divergidas del repo (la versión viva en prod ya
-- tenía costo_distancia + rechazos, no versionados). Este archivo deja la
-- definición COMPLETA vigente para volver a sincronizar.
...
AS $$
  ...
  base as (
    select vd.id_cliente, max(vd.nombre_cliente) nombre_cliente,
           ...
           count(distinct vd.fecha::text || '|' || coalesce(vd.ds_fletero_carga,''))::int comprobantes,
