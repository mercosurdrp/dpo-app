-- =============================================
-- Períodos Críticos — vista multi-año
--
-- v_pc_calendario_dia_multianio: misma lógica que v_pc_calendario_dia pero
-- devuelve los días de varios años en una sola tabla con columna `anio`.
-- Permite mostrar 2025 y 2026 lado a lado en la UI sin tocar pc_config.
--
-- Cobertura: 2024 → año actual + 1 (configurable abajo).
-- =============================================
CREATE OR REPLACE VIEW v_pc_calendario_dia_multianio AS
WITH cfg AS (
  SELECT
    c.w_vol, c.w_otif, c.w_aus,
...
anios AS (
  -- 2024 hasta año actual + 1 (preparado para cargar siguiente año)
  SELECT generate_series(2024, EXTRACT(year FROM CURRENT_DATE)::int + 1) AS anio
),
fechas AS (
  SELECT a.anio, generate_series(make_date(a.anio,1,1), make_date(a.anio,12,31), interval '1 day')::date AS fecha
  FROM anios a
),
ventas_dia AS (
  SELECT fecha, SUM(total_hl)::numeric AS hl_real, COUNT(DISTINCT ds_fletero_carga) AS camiones
...
SELECT
  ...
  END AS dia_semana,
  s.mes,
  s.hl, s.hl_rechazo, s.camiones, s.clientes_dia,
  s.pct_rechazo, s.otif_estimado, s.pct_ausentismo, s.clasif_vol,
  s.nombre_feriado IS NOT NULL AS es_feriado, s.nombre_feriado,
  s.score,
