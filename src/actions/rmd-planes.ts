...
export type EstadoRmdPlan = "pendiente" | "en_progreso" | "completado"
export type PrioridadRmdPlan = "alta" | "media" | "baja"

const ESTADOS_VALIDOS: EstadoRmdPlan[] = [
  "pendiente",
  "en_progreso",
  "completado",
]
const PRIORIDADES_VALIDAS: PrioridadRmdPlan[] = ["alta", "media", "baja"]

type Result<T> = { data: T } | { error: string }

export type RecuperacionRmdPlan =
  | "recuperado" // RMD posterior alto (≥4.5)
  | "mejorando" // RMD posterior subió respecto del baseline pero <4.5
  | "critico" // RMD posterior igual o peor que el baseline
  | "sin_reencuesta" // todavía no hay nuevas puntuaciones desde el plan

export interface RmdPlan {
  id: string
  titulo: string
  descripcion: string | null
  foco_motivo: string | null
  ...
  // --- seguimiento del RMD del cliente foco ---
  baseline_rmd: number | null
  baseline_n: number | null
  baseline_fecha: string | null
  recuperacion: RecuperacionRmdPlan | null
  /** RMD del cliente desde que existe el plan. */
  post_rmd_n: number
  post_rmd_avg: number | null
  post_rmd_fecha: string | null
}

export interface RmdPlanAvance {
  id: string
  plan_id: string
  ...
  estado_resultante: EstadoRmdPlan | null
...
// ------------------------------------------------------------------
export async function listarPlanesRmd(): Promise<Result<RmdPlan[]>> {
  try {
    await requireAuth()
    ...
    const { data, error } = await supabase
      .from("rmd_planes")
      .select(
        "*, responsable:profiles!rmd_planes_responsable_id_fkey(id, nombre), autor:profiles!rmd_planes_created_by_fkey(id, nombre)",
      )
    ...
    // Seguimiento: RMD posterior al plan para todos los clientes foco.
    const clienteIds = [
      ...new Set(
        rows
    ...
    if (clienteIds.length) {
      const { data: rmdRes } = await supabase
      ...
      for (const r of (rmdRes ?? []) as Array<{
        cod_cliente: number
    ...
    const planes: RmdPlan[] = rows.map((row) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const r = row as any

      let recuperacion: RecuperacionRmdPlan | null = null
      let post_rmd_n = 0
      let post_rmd_avg: number | null = null
      let post_rmd_fecha: string | null = null
      if (r.foco_cliente_id != null) {
        ...
        if (rmds.length === 0) {
          ...
          post_rmd_avg = avg
          post_rmd_fecha = rmds[rmds.length - 1].fecha_puntuacion
          const base = (r.baseline_rmd as number | null) ?? null
          if (avg >= 4.5) recuperacion = "recuperado"
          else if (base != null && avg > base) recuperacion = "mejorando"
      ...
      return {
        ...
        foco_motivo: r.foco_motivo ?? null,
        ...
        baseline_rmd: r.baseline_rmd ?? null,
...
// Crear plan
// ------------------------------------------------------------------
export async function crearPlanRmd(
  formData: FormData,
  ...
  try {
    ...
    const prioridad = PRIORIDADES_VALIDAS.includes(
      prioridadRaw as PrioridadRmdPlan,
    )
      ? (prioridadRaw as PrioridadRmdPlan)
      : "media"

    const focoMotivo = String(formData.get("foco_motivo") ?? "").trim() || null
    ...
    // Baseline: el RMD del cliente foco hasta el momento de crear el plan,
    // para después medir la mejora contra las puntuaciones posteriores.
    let baseline: {
      baseline_rmd?: number
      baseline_n?: number
    ...
    if (focoClienteId != null) {
      const { data: pts } = await supabase
        .from("nps_rmd_cliente")
        .select("puntuacion, fecha_puntuacion")
        .eq("cod_cliente", focoClienteId)
        .order("fecha_puntuacion", { ascending: true })
      const arr = (pts ?? []) as Array<{
        puntuacion: number
        fecha_puntuacion: string
      }>
      if (arr.length) {
        const avg =
          Math.round(
            (arr.reduce((s, x) => s + x.puntuacion, 0) / arr.length) * 100,
          ) / 100
        baseline = {
          baseline_rmd: avg,
          baseline_n: arr.length,
          baseline_fecha: arr[arr.length - 1].fecha_puntuacion,
        }
      }
    }
...
// ------------------------------------------------------------------
export async function actualizarPlanRmd(
  planId: string,
  formData: FormData,
  ...
  try {
    ...
    if (formData.has("prioridad")) {
      ...
      if (PRIORIDADES_VALIDAS.includes(pr as PrioridadRmdPlan))
        updates.prioridad = pr
    }
    if (formData.has("estado")) {
      const es = String(formData.get("estado") ?? "").trim()
      if (!ESTADOS_VALIDOS.includes(es as EstadoRmdPlan))
        return { error: "Estado inválido" }
      updates.estado = es
    }
    if (formData.has("foco_motivo"))
      updates.foco_motivo =
        String(formData.get("foco_motivo") ?? "").trim() || null
...
// ------------------------------------------------------------------
export async function eliminarPlanRmd(
  planId: string,
): Promise<Result<{ ok: true }>> {
  try {
    const profile = await requireAuth()
...
// ------------------------------------------------------------------
export async function listarAvancesPlanRmd(
  planId: string,
): Promise<Result<RmdPlanAvance[]>> {
  try {
    await requireAuth()
    const supabase = await createClient()
    if (!planId) return { error: "ID de plan inválido" }
    ...
    const { data, error } = await supabase
      .from("rmd_planes_avances")
      .select(
        "*, autor:profiles!rmd_planes_avances_autor_id_fkey(id, nombre)",
      )
      .eq("plan_id", planId)
      .order("created_at", { ascending: false })
    if (error) return { error: error.message }

    const avances: RmdPlanAvance[] = (
      ...
      return {
        ...
        estado_resultante: (r.estado_resultante as EstadoRmdPlan | null) ?? null,
...
export async function agregarAvancePlanRmd(
  planId: string,
  formData: FormData,
): Promise<Result<RmdPlanAvance>> {
  try {
    ...
    let nuevoEstado: EstadoRmdPlan | null = null
    if (nuevoEstadoRaw) {
      if (!ESTADOS_VALIDOS.includes(nuevoEstadoRaw as EstadoRmdPlan))
        return { error: "Estado inválido" }
      nuevoEstado = nuevoEstadoRaw as EstadoRmdPlan
    }
    ...
    const { data: avance, error: errAv } = await supabase
      ...
      .select("*, autor:profiles!rmd_planes_avances_autor_id_fkey(id, nombre)")
      .single()
...
export async function eliminarAvancePlanRmd(
  avanceId: string,
): Promise<Result<{ ok: true }>> {
  try {
    const profile = await requireAuth()
...
export async function getAvanceRmdSignedUrl(
  archivoPath: string,
