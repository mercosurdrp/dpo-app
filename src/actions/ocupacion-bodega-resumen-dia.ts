...
export interface ViajeDelDia {
  ...
  /** % de la bodega ocupado (sobre 1440): el dato físico. */
  ob_pct: number
  /** % del objetivo de carga alcanzado (sobre 600): el número de gestión. */
...
export interface OBResumenDia {
  ...
  ceq_total: number
  ceq_promedio: number
  pct_promedio: number   // promedio simple de ocupación por viaje (sobre la capacidad)
  cumplimiento_promedio: number  // promedio simple de cumplimiento por viaje (sobre el objetivo)
...
export async function getOcupacionBodegaResumenDia(
  ...
  try {
    ...
    if (rows.length === 0) {
      return {
        data: {
          fecha, capacidad: CAPACIDAD_CEQ, minimo: OBJETIVO_CEQ,
          total_viajes: 0, ceq_total: 0, ceq_promedio: 0,
          pct_promedio: 0, cumplimiento_promedio: 0, en_meta: 0, patente_top: null, ceq_max: 0, ceq_min: 0,
          viajes: [],
        },
      }
    }
    const viajes: ViajeDelDia[] = rows.map(r => ({
      ...
      cumplimiento_pct: cumplimientoPct(r.ceq_total),
    }))
    ...
    const cumProm = viajes.reduce((a, v) => a + v.cumplimiento_pct, 0) / viajes.length
    const enMeta = viajes.filter(v => alcanzaObjetivo(v.ceq_total)).length
    return {
      data: {
        fecha,
        ...
        cumplimiento_promedio: Math.round(cumProm * 10) / 10,
        en_meta: enMeta,
