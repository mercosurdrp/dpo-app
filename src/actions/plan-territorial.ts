...
// real. Es el mismo motor que el costo vigente: lo único que cambia son los km.
// ==================================================================
...
export interface Simulacion {
  ...
  delta_pct: number
  /**
   * Peso del componente de distancia, antes y después.
   *
   * 🚨 Ojo con interpretar `delta_pct`: el modelo parte de un pool de costo FIJO
   * (lo que Finanzas cargó en costo_logistico_mensual) y lo reparte entre los
   * PDV. Cambiar los km cambia *cómo se reparte*, no cuánto se gasta, así que el
   * VLC/HL total da idéntico y `delta_pct` es siempre ~0.
   *
   * Lo que sí se mueve es esto: cuánto pesa la distancia en el reparto. Es la
   * señal de que el territorio se acomoda mejor, pero NO es un ahorro: para
   * convertirlo en pesos hay que valorizar los km evitados a $/km y bajar el
   * pool, que es una decisión de modelo pendiente.
   */
...
export async function simularRelocalizacion(
  ...
  try {
    ...
    const sumar = (m: Map<string, { hl: number; costo: number; dist: number }>) =>
      [...m.values()].reduce(
        (t, v) => ({
          hl: t.hl + v.hl,
    ...
    return {
      error: err instanceof Error ? err.message : "Error corriendo la simulación",
    }
  }
}
