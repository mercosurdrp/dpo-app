// Planes de acción del indicador Tiempo por Punto de Venta.
//
// Derivado de `tlp-planes.ts` (mismo modelo de datos y RLS). La diferencia es el
// foco: acá el plan se ata al PDV concreto (`foco_cliente_id`), que es la unidad
// sobre la que se puede actuar — hablar con el cliente, mover la ventana horaria,
// revisar el acceso. Ciudad y patente quedan como foco opcional adicional.
...
const BUCKET = "tiempo-pdv-planes"
const TPDV_PATH = "/indicadores/tiempo-pdv"

export type EstadoTiempoPdvPlan = "pendiente" | "en_progreso" | "completado"
export type PrioridadTiempoPdvPlan = "alta" | "media" | "baja"

const ESTADOS_VALIDOS: EstadoTiempoPdvPlan[] = ["pendiente", "en_progreso", "completado"]
const PRIORIDADES_VALIDAS: PrioridadTiempoPdvPlan[] = ["alta", "media", "baja"]

type Result<T> = { data: T } | { error: string }

export interface TiempoPdvPlan {
  id: string
  titulo: string
  descripcion: string | null
  /** PDV al que apunta el plan: el foco principal de este indicador. */
  foco_cliente_id: string | null
  foco_cliente: string | null
  ...
  prioridad: PrioridadTiempoPdvPlan
  estado: EstadoTiempoPdvPlan
...
export interface TiempoPdvPlanAvance {
  id: string
  plan_id: string
  ...
  estado_resultante: EstadoTiempoPdvPlan | null
  autor_id: string | null
  autor_nombre: string | null
  created_at: string
}

export interface TiempoPdvPlanFiltro {
...
// ------------------------------------------------------------------
export async function listarPlanesTiempoPdv(
  filtro?: TiempoPdvPlanFiltro,
): Promise<Result<TiempoPdvPlan[]>> {
  try {
    await requireAuth()
    const supabase = await createClient()

    let q = supabase
      .from("tiempo_pdv_planes")
      .select(
        "*, responsable:profiles!tiempo_pdv_planes_responsable_id_fkey(id, nombre), autor:profiles!tiempo_pdv_planes_created_by_fkey(id, nombre)",
      )
      .order("created_at", { ascending: false })

    if (filtro?.foco_cliente_id) q = q.eq("foco_cliente_id", filtro.foco_cliente_id)
    ...
    if (ids.length) {
      const { data: avs } = await supabase
        .from("tiempo_pdv_planes_avances")
        .select("plan_id")
        .in("plan_id", ids)
    ...
    const planes: TiempoPdvPlan[] = rows.map((row) => {
      ...
      return {
        ...
        foco_cliente: r.foco_cliente ?? null,
        foco_ciudad: r.foco_ciudad ?? null,
        foco_patente: r.foco_patente ?? null,
        prioridad: (r.prioridad as PrioridadTiempoPdvPlan) ?? "media",
        estado: (r.estado as EstadoTiempoPdvPlan) ?? "pendiente",
...
// Crear plan
// ------------------------------------------------------------------
export async function crearPlanTiempoPdv(
  formData: FormData,
  ...
  try {
    ...
    const prioridad = PRIORIDADES_VALIDAS.includes(prioridadRaw as PrioridadTiempoPdvPlan)
      ? (prioridadRaw as PrioridadTiempoPdvPlan)
      : "media"

    const focoClienteId = String(formData.get("foco_cliente_id") ?? "").trim() || null
    const focoCliente = String(formData.get("foco_cliente") ?? "").trim() || null
...
// ------------------------------------------------------------------
export async function actualizarPlanTiempoPdv(
  planId: string,
  formData: FormData,
  ...
  try {
    ...
    if (formData.has("prioridad")) {
      ...
      if (PRIORIDADES_VALIDAS.includes(pr as PrioridadTiempoPdvPlan)) updates.prioridad = pr
    }
    if (formData.has("estado")) {
      const es = String(formData.get("estado") ?? "").trim()
      if (!ESTADOS_VALIDOS.includes(es as EstadoTiempoPdvPlan))
        return { error: "Estado inválido" }
      updates.estado = es
    }
    if (formData.has("foco_cliente_id"))
      updates.foco_cliente_id =
        String(formData.get("foco_cliente_id") ?? "").trim() || null
    if (formData.has("foco_cliente"))
      updates.foco_cliente = String(formData.get("foco_cliente") ?? "").trim() || null
...
// ------------------------------------------------------------------
export async function eliminarPlanTiempoPdv(
  planId: string,
): Promise<Result<{ ok: true }>> {
  try {
    const profile = await requireAuth()
    ...
    const { error } = await supabase.from("tiempo_pdv_planes").delete().eq("id", planId)
    if (error) return { error: error.message }
...
// ------------------------------------------------------------------
export async function listarAvancesPlanTiempoPdv(
  planId: string,
): Promise<Result<TiempoPdvPlanAvance[]>> {
  try {
    await requireAuth()
    const supabase = await createClient()
    if (!planId) return { error: "ID de plan inválido" }
    ...
    const { data, error } = await supabase
      .from("tiempo_pdv_planes_avances")
      .select("*, autor:profiles!tiempo_pdv_planes_avances_autor_id_fkey(id, nombre)")
      .eq("plan_id", planId)
      .order("created_at", { ascending: false })
    if (error) return { error: error.message }

    const avances: TiempoPdvPlanAvance[] = (
      ...
      return {
        ...
        estado_resultante: (r.estado_resultante as EstadoTiempoPdvPlan | null) ?? null,
...
export async function agregarAvancePlanTiempoPdv(
  planId: string,
  formData: FormData,
): Promise<Result<TiempoPdvPlanAvance>> {
  try {
    ...
    let nuevoEstado: EstadoTiempoPdvPlan | null = null
    if (nuevoEstadoRaw) {
      if (!ESTADOS_VALIDOS.includes(nuevoEstadoRaw as EstadoTiempoPdvPlan))
        return { error: "Estado inválido" }
      nuevoEstado = nuevoEstadoRaw as EstadoTiempoPdvPlan
    }
...
export async function eliminarAvancePlanTiempoPdv(
  avanceId: string,
): Promise<Result<{ ok: true }>> {
  try {
    const profile = await requireAuth()
...
export async function getAvanceTiempoPdvSignedUrl(
  archivoPath: string,
