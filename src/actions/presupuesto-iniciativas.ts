...
const AREAS_VALIDAS: AreaIniciativaAhorro[] = [
  "distribucion",
  "gente",
  "operaciones",
  "otro",
  ...
  try {
    ...
    const enriched: IniciativaAhorroConDetalle[] = (data ?? []).map((row) => {
      ...
      return {
        ...
        area: (r.area ?? null) as AreaIniciativaAhorro | null,
        inversion_capex:
          r.inversion_capex !== null ? Number(r.inversion_capex) : null,
        nivel_impacto:
          r.nivel_impacto !== null ? Number(r.nivel_impacto) : null,
        acciones_clave: r.acciones_clave ?? null,
...
function camposIniciativaDesdeForm(formData: FormData): Record<string, any> {
  ...
  const area: AreaIniciativaAhorro | null = AREAS_VALIDAS.includes(
    areaRaw as AreaIniciativaAhorro,
  )
    ? (areaRaw as AreaIniciativaAhorro)
    : null
  ...
  return {
    ...
    // Campos extra (plantilla Misiones)
    area,
    inversion_capex: parseNum(formData.get("inversion_capex")),
    nivel_impacto: parseNum(formData.get("nivel_impacto")),
    acciones_clave: parseText(formData.get("acciones_clave")),
    justificacion: parseText(formData.get("justificacion")),
  }
}
