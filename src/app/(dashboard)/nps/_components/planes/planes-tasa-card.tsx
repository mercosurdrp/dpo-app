...
interface Props {
  ...
  resumen: NpsCoberturaData["resumen"]
  /** Meses del año, para ver cómo viene la tasa desde que existe el plan. */
  meses: NpsCoberturaMes[]
  onNuevo: () => void
  onVer: (plan: NpsPlan) => void
}

/**
 * Tarjeta de la solapa Cobertura con los planes que atacan la tasa de
 * respuesta de la encuesta NPS (por ejemplo, el folleto que se deja en los
 * PDV) y el objetivo que persiguen. Los planes se abren acá mismo, sin
 * saltar a la solapa del panel. Espejo de la tarjeta de RMD.
 */
export function PlanesTasaCard({
  ...
  const brecha = brechaTasaNps(tasa)
  const logrado = brecha != null && brecha === 0
  const faltantes = respuestasFaltantesNps(
    resumen.enviadas,
    resumen.respondidas,
  )
  const mesesCerrados = meses.filter((m) => !m.parcial && m.enviadas > 0)
  // Cuánto representa la brecha en un mes típico: respuestas que tendrían
  ...
  const desdePlan = useMemo(() => {
    ...
    const respondidas = post.reduce((s, m) => s + m.respondidas, 0)
    return {
      mesPlan,
      tasa: enviadas > 0 ? (respondidas / enviadas) * 100 : null,
  ...
  const avance =
    tasa == null ? 0 : Math.min(100, (tasa / NPS_TASA_OBJETIVO) * 100)
    ...
    <Card className="border-amber-200">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-slate-500">
          Acciones generales sobre toda la base para que más clientes respondan
          la encuesta. Clic en un plan para ver el detalle, cargar avances o
          cambiarle el estado.
        </p>
      </CardHeader>
      <CardContent className="space-y-4 border-t pt-4">
        ...
        >
          <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
            <span className="flex items-center gap-1.5 text-sm font-semibold text-slate-800">
              ...
              Objetivo: llegar al {NPS_TASA_OBJETIVO} % de tasa de respuesta
            </span>
          ...
          <div
            ...
            title={`${tasa == null ? "—" : `${fmt(tasa, 1)} %`} de ${NPS_TASA_OBJETIVO} %`}
          >
            <div
          ...
          <div className="mt-2 grid gap-1 text-xs text-slate-600 sm:grid-cols-2">
            <p>
              Meses cerrados del año: {fmt(resumen.respondidas)} respondidas de{" "}
              {fmt(resumen.enviadas)} enviadas.
              {!logrado && faltantes > 0 && (
                <>
                  {" "}
                  Para el {NPS_TASA_OBJETIVO} % hacían falta{" "}
            ...
            <p>
              {desdePlan == null ? (
                ...
                <>
                  ...
                  <strong
                    className={
                      desdePlan.tasa >= NPS_TASA_OBJETIVO
                        ? "text-emerald-700"
                        : "text-slate-800"
                    }
                  >
                  ...
                  {desdePlan.parcial && (
                    <>
                      {" "}
                      · parcial, el Power BI todavía no cargó todas las
                      enviadas del mes
                    </>
                  )}
                  .
        ...
        {planes.length === 0 ? (
          <p className="py-6 text-center text-sm text-slate-400">
            Todavía no hay un plan para llegar al {NPS_TASA_OBJETIVO} %. Crealo
            desde «Nuevo plan».
          </p>
        ) : (
