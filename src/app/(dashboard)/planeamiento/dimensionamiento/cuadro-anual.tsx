        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {fila(<>Días con 2ª vuelta <span className="text-xs font-normal text-muted-foreground">(más de {camDisp} camiones)</span></>, (c) => { const v = diasRefuerzo(c); if (v == null) return dash; return est(c, v > 0 ? <span className="font-semibold text-amber-700">{v}</span> : "—") })}
              {fila("Cantidad de segundas vueltas", (c) => { const v = camiones(c); return v == null ? dash : est(c, v.vueltas > 0 ? <span className="font-semibold text-red-700">{fmt(v.vueltas)}</span> : "—") }, true)}
              {fila(<>Días con toda la flota <span className="text-xs font-normal text-muted-foreground">(los {camDisp}, sin margen)</span></>, (c) => { if (c.tipo === "cerrado" || c.pi < 0) return dash; const fc = diasFlotaCompleta(c); return fc > 0 ? <span className="font-semibold text-amber-700">{fc}</span> : "—" }, true)}
