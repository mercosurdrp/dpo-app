...
export function CuadroAnualCard({ data, proy }: { data: DimData; proy: ProyeccionData | null }) {
  ...
  const cerrados = cols.filter((c) => c.tipo === "cerrado")
    ...
    <Card>
      ...
      <CardContent>
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {fila("Año anterior", (c) => { const v = esc(c.m)?.aa; return v == null ? dash : fmt(v) })}
              {fila("Presupuesto", (c) => { const v = esc(c.m)?.presupuesto; return v == null ? dash : fmt(v) })}
              ...
              {fila(<b>Real</b>, (c) => { const e = esc(c.m); return e?.real == null ? dash : <b>{fmt(e.real)}{e.parcial ? "*" : ""}</b> })}
              {fila("Real vs presupuesto", (c) => { const e = esc(c.m); if (!e || e.parcial) return dash; const v = pct(e.real, e.presupuesto); return <span className={v != null && v < -0.1 ? "text-red-700" : v != null && v > 0.1 ? "text-sky-700" : ""}>{fmtPct(v)}</span> }, true)}
              {fila("Real vs año anterior", (c) => { const e = esc(c.m); if (!e || e.parcial) return dash; return fmtPct(pct(e.real, e.aa)) }, true)}
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          ...
          Volumen real y año anterior = HL distribuidos con flota propia (Chess + GESCOM). «nec. / dotación» = necesarios en el día promedio (almacén: ÷ (1 − ausentismo)) contra la dotación nominal; <span className="text-red-700">faltan</span> = temporales requeridos, <span className="text-sky-700">sobran</span> en azul = ocupación por debajo del {Math.round(umbral * 100)} % (capacidad ociosa).
