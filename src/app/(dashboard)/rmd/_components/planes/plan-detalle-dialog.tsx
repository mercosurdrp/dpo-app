...
import {
  ...
  eliminarAvancePlanRmd,
  eliminarPlanRmd,
  getAvanceRmdSignedUrl,
  listarAvancesPlanRmd,
  type EstadoRmdPlan,
  type RmdPlan,
  type RmdPlanAvance,
...
export function PlanDetalleDialog({
  ...
  const [avances, setAvances] = useState<RmdPlanAvance[]>([])
  ...
  async function recargarAvances() {
    ...
    const r = await listarAvancesPlanRmd(plan.id)
  ...
  // Preview de imágenes con signed URL. Se cachean POR PATH: un avance puede
  // traer varias imágenes.
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (const arch of pendientes) {
        const r = await getAvanceRmdSignedUrl(arch.path)
  ...
  async function handleAbrirArchivo(path: string, nombre: string | null) {
    const r = await getAvanceRmdSignedUrl(path)
    if ("error" in r) {
      toast.error(r.error)
      return
    }
  ...
  function handleAgregarAvance() {
    ...
    startSubmit(async () => {
      const r = await agregarAvancePlanRmd(plan.id, fd)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  function handleEliminarAvance(id: string) {
    ...
    startSubmit(async () => {
      const r = await eliminarAvancePlanRmd(id)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  function handleEliminarPlan() {
    if (
      ...
      return
    startEliminar(async () => {
      const r = await eliminarPlanRmd(plan.id)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        ...
        {/* Recuperación del cliente foco (RMD base vs. RMD posterior al plan) */}
        {plan.foco_cliente_id != null && (
          <div className="space-y-1.5 rounded-md border border-slate-200 bg-slate-50/60 p-3 text-sm">
            ...
            <p className="text-slate-600">
              Punto de partida:{" "}
              {plan.baseline_rmd != null ? (
                <>
                  <span className="font-medium">
                    RMD base {plan.baseline_rmd.toFixed(2)} / 5
                  </span>{" "}
                  ({plan.baseline_n ?? 0}{" "}
                  {plan.baseline_n === 1 ? "entrega" : "entregas"} hasta{" "}
                  {fechaDia(
                    plan.baseline_fecha
                      ? plan.baseline_fecha.slice(0, 10)
                      : null,
                  )}
                  )
                </>
              ) : (
                <span className="font-medium">Sin RMD previo</span>
              )}
            </p>
            <p className="text-slate-600">
              {plan.post_rmd_n > 0 ? (
                <>
                  ...
                  <span
                    className={`font-medium ${
                      (plan.post_rmd_avg ?? 0) >= 4.5
                        ? "text-emerald-700"
                        : (plan.post_rmd_avg ?? 0) >= 4
                          ? "text-amber-700"
                          : "text-red-700"
                    }`}
                  >
                    {plan.post_rmd_avg?.toFixed(2)} / 5
                  </span>{" "}
                  ({plan.post_rmd_n}{" "}
                  {plan.post_rmd_n === 1 ? "entrega" : "entregas"})
                  {plan.post_rmd_fecha &&
                    ` · última el ${fechaDia(plan.post_rmd_fecha.slice(0, 10))}`}
                </>
              ) : (
                <>
                  El cliente todavía no puntuó entregas desde que existe el
                  plan.
                </>
              )}
            </p>
