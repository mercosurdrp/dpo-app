...
import {
  ...
  updateTiPlanItem,
  deleteTiPlanItem,
  updateTiPlanCausaRaiz,
  cerrarTiPlan,
  deleteTiPlan,
} from "@/actions/tiempo-interno"
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-start justify-between">
          <div>
            <CardTitle className="text-base">Planes de Acción Tiempo Interno (R1.3.4)</CardTitle>
        ...
        <CardContent>
          {resumen.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">
              Sin datos de Tiempo Interno todavía.
            </p>
          ) : (
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-muted-foreground">
          TI {resumen.promedio_ti} min · {resumen.pct_dentro_meta}% dentro meta
          {plan && (
            <>
              {" "}
...
function CreatePlanDialog({
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const result = await createTiPlan({
      mes: resumen.mes,
      year: resumen.year,
      promedioTiMes: resumen.promedio_ti,
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            TI {resumen.promedio_ti} min · {resumen.pct_dentro_meta}% dentro de meta
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Textarea
              rows={4}
              placeholder="Por qué el Tiempo Interno del mes no cumplió la meta. Aplicá la técnica de los 5 porqués."
              value={causaRaiz}
...
function PlanDetailDialog({
  ...
  const [items, setItems] = useState<TiPlanAccionItem[]>([])
  ...
  async function handleSaveCausa() {
    setBusy(true)
    const r = await updateTiPlanCausaRaiz(planId, causaEdit)
    setBusy(false)
    if ("error" in r) {
      toast.error(r.error)
      return
  ...
  async function handleItemEstado(id: string, estado: PlanTiItemEstado) {
    const r = await updateTiPlanItem({ id, estado })
    if ("error" in r) {
      toast.error(r.error)
      return
    }
  ...
  async function handleItemDelete(id: string) {
    const r = await deleteTiPlanItem(id)
    if ("error" in r) {
      toast.error(r.error)
      return
    }
  ...
  async function handleCerrarPlan() {
    ...
    setBusy(true)
    const r = await cerrarTiPlan(planId, resultadoCierre, evidenciaCierre || null)
    setBusy(false)
    if ("error" in r) {
      toast.error(r.error)
      return
  ...
  async function handleDeletePlan() {
    ...
    setBusy(true)
    const r = await deleteTiPlan(planId)
    setBusy(false)
    if ("error" in r) {
      toast.error(r.error)
      return
                    ...
                    {!cerrado ? (
                      <Select
                        ...
                        onValueChange={(v: string | null) =>
                          v && handleItemEstado(it.id, v as PlanTiItemEstado)
                        }
                      >
