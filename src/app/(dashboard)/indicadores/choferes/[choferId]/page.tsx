...
import { getChoferDetalle } from "@/actions/choferes"
import { ChoferDetalleClient } from "./chofer-detalle-client"
...
function parseRango(
  ...
  if (desde > hasta) return { desde: defaultDesde, hasta: defaultHasta }
  return { desde, hasta }
}
...
export default async function ChoferDetallePage({
  ...
  params: Promise<{ choferId: string }>
  searchParams: Promise<Record<string, string | string[] | undefined>>
}) {
  const { choferId } = await params
  const sp = await searchParams
  const { desde, hasta } = parseRango(sp)
  const res = await getChoferDetalle(decodeURIComponent(choferId), desde, hasta)

  const volverParams = new URLSearchParams()
  volverParams.set("desde", desde)
  volverParams.set("hasta", hasta)

  return (
    <div className="space-y-4">
      <Link
        href={`/indicadores/choferes?${volverParams.toString()}`}
      ...
      {"error" in res ? (
        ...
        <ChoferDetalleClient data={res.data} desde={desde} hasta={hasta} />
      )}
    </div>
  )
}
