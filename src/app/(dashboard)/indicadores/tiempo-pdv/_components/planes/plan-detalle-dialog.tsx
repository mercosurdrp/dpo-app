...
import {
  agregarAvancePlanTiempoPdv,
  eliminarAvancePlanTiempoPdv,
  eliminarPlanTiempoPdv,
  getAvanceTiempoPdvSignedUrl,
  listarAvancesPlanTiempoPdv,
  type EstadoTiempoPdvPlan,
  type TiempoPdvPlan,
  type TiempoPdvPlanAvance,
...
export function PlanDetalleDialog({
  ...
  const [avances, setAvances] = useState<TiempoPdvPlanAvance[]>([])
  ...
  async function recargarAvances() {
    ...
    const r = await listarAvancesPlanTiempoPdv(plan.id)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (const arch of pendientes) {
        const r = await getAvanceTiempoPdvSignedUrl(arch.path)
  ...
  async function handleAbrirArchivo(path: string, nombre: string | null) {
    const r = await getAvanceTiempoPdvSignedUrl(path)
    if ("error" in r) {
      toast.error(r.error)
      return
    }
  ...
  function handleAgregarAvance() {
    ...
    startSubmit(async () => {
      const r = await agregarAvancePlanTiempoPdv(plan.id, fd)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  function handleEliminarAvance(id: string) {
    ...
    startSubmit(async () => {
      const r = await eliminarAvancePlanTiempoPdv(id)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  function handleEliminarPlan() {
    if (
      ...
      return
    startEliminar(async () => {
      const r = await eliminarPlanTiempoPdv(plan.id)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
          ...
          {(plan.foco_ciudad || plan.foco_patente) && (
            <div className="flex items-start gap-2">
              ...
              <span>
                ...
                {[
                  plan.foco_ciudad ? `Ciudad: ${plan.foco_ciudad}` : null,
                  plan.foco_patente ? `Camión: ${plan.foco_patente}` : null,
                ]
                  .filter(Boolean)
                  .join(" · ")}
              </span>
