...
interface Props {
  ...
  planExistente?: TlpPlan | null
  focoInicial?: { foco_ciudad?: string; foco_patente?: string } | null
  onSaved: () => void
}
...
export function PlanFormDialog({
  ...
  const [prioridad, setPrioridad] = useState<PrioridadTlpPlan>("media")
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const r = esEdicion
        ? await actualizarPlanTlp(planExistente!.id, fd)
        : await crearPlanTlp(fd)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
