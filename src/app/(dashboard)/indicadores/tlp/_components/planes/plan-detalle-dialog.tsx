...
import {
  agregarAvancePlanTlp,
  eliminarAvancePlanTlp,
  eliminarPlanTlp,
  getAvanceTlpSignedUrl,
  listarAvancesPlanTlp,
  type EstadoTlpPlan,
  type TlpPlan,
  type TlpPlanAvance,
...
export function PlanDetalleDialog({
  ...
  const [avances, setAvances] = useState<TlpPlanAvance[]>([])
  ...
  async function recargarAvances() {
    ...
    const r = await listarAvancesPlanTlp(plan.id)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (const arch of pendientes) {
        const r = await getAvanceTlpSignedUrl(arch.path)
  ...
  async function handleAbrirArchivo(path: string, nombre: string | null) {
    const r = await getAvanceTlpSignedUrl(path)
    if ("error" in r) {
      toast.error(r.error)
      return
    }
  ...
  function handleAgregarAvance() {
    ...
    startSubmit(async () => {
      const r = await agregarAvancePlanTlp(plan.id, fd)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  function handleEliminarAvance(id: string) {
    ...
    startSubmit(async () => {
      const r = await eliminarAvancePlanTlp(id)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
  ...
  function handleEliminarPlan() {
    if (
      ...
      return
    startEliminar(async () => {
      const r = await eliminarPlanTlp(plan.id)
      if ("error" in r) {
        toast.error(r.error)
        return
      }
